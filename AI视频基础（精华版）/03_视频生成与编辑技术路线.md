...
这道题考的是分类是否有统一标准。列出多少任务缩写不重要，关键要说清输入已知什么、模型还要补全什么，以及输出必须保留什么。

按源视觉信息划分，最常见的是 T2V、I2V 和 V2V，三者的任务边界如表3-1所示。
...
| **I2V** | 首帧、末帧或参考图 | 帧间运动、遮挡后的内容和画外区域 | 参考外观、初始结构或指定端点 |
| **V2V** | 源视频，通常再加编辑指令或 mask | 被修改的外观、语义、局部内容或运动 | 原始时序骨架和非编辑内容 |

三者的区别可以统一写成：

```math
p\left(y \mid s, c, m, \tau\right)
```

$y$ 是输出视频，$s$ 是可选的源图像或源视频，$c$ 是文本、音频、姿态、轨迹等条件，$m$ 限定编辑区域，$\tau$ 表示生成、续写、编辑或修复等任务类型。T2V、I2V、V2V 主要由 $s$ 决定。

Subject-to-Video、Audio-driven、Pose Control 描述的是额外控制，Joint Audio-Video 描述输出模态，Unified Multimodal Generation 描述系统如何组织这些能力。它们不在同一条分类轴上。例如，给定人物参考图和动作轨迹生成视频，主范式属于 I2V，同时带有主体和轨迹控制。
...
把视频表示成 latent/token 时空网格后，可以用两类作用域区分这四类任务。$M_{\mathrm{obs}}$ 标记可信的输入位置，$M_{\mathrm{write}}$ 标记允许生成或改写的位置；编辑指令 $c_{\mathrm{edit}}$ 再规定目标区域要发生什么变化。$M_{\mathrm{write}}$ 可以是显式时空 mask，也可以由指令隐式推断。三者的关系如表3-2所示。

<p align="center">表3-2 视频续写、关键帧条件生成、视频修复与视频编辑的任务边界</p>

| 任务 | $M_{\mathrm{obs}}$ | $M_{\mathrm{write}}$ | 底层目标与典型失败 |
| --- | --- | --- | --- |
...
以参考图为例，通用图像编码器可能保留了“人”或“狗”这类高层语义，却丢掉区分具体主体的细节。即使编码结果中有这些细节，条件特征如果只在少数网络层或时间步注入，影响也可能随去噪和时间推进而衰减。训练数据还需要建立“同一主体、不同姿态与视角”的对应关系，否则模型往往只能生成语义相似的主体。

验证时应固定随机种子和其他条件，仅替换、移除或打乱待测条件。如果目标属性几乎不变，说明条件可能被忽略；如果无关区域同步大幅变化，说明条件的作用域没有被控制住。这两类问题都不能由整体画质分数代替评估。
...
分析技术路线时，沿视频在系统中的实际数据流往下看，比记模型名称更可靠：像素视频先被压缩成 latent 或 token，生成主干在压缩空间中建模时空关系，外部条件通过特定路径约束去噪，再经过采样和解码还原视频。

视频表示决定信息上限和计算底座。空间、时间压缩率越高，主干处理的 token 越少，但小物体、快速运动和短时事件可能在进入主干前已经丢失；压缩率过低，注意力计算和显存又会随分辨率、帧数迅速增加。如果重建视频本身就模糊或运动断裂，应先检查 Video VAE / tokenizer，再检查生成主干。

主干部分要看时空信息如何交换。2D 主干外挂 temporal layer、空间与时间分解注意力、局部窗口和全时空注意力，差别主要在时序感受野、计算复杂度，以及空间先验与运动建模的耦合程度。单帧质量正常但动作不连贯时，应优先检查时间通路、训练 clip 的长度分布和帧率，而非继续扩大空间主干。

条件路径决定可控性。参考图、文本、轨迹或源视频在什么网络层、去噪阶段和时空区域生效，会直接影响条件能否被遵循，以及无关内容能否保持。只增加一个输入接口，无法补上训练数据和学习目标中没有建立的条件—结果对应关系。

还要核对训练和推理是否匹配，包括视频长度、帧率、分辨率、任务样本与条件覆盖范围。训练只见过短 clip 的模型，即使推理时能接收更长 token 序列，也可能在长时间范围内出现状态漂移。读一条技术路线，最终要能指出信息在哪里被压缩、跨帧依赖在哪里建立、条件在哪里生效，以及故障首先该查哪一层。
...
图像模型视频化通常保留大部分 T2I 的 VAE、U-Net 或 DiT 空间通路，再插入 temporal attention、3D convolution、Motion Module，或用图像权重初始化新增的时间结构。它能直接继承图像预训练获得的纹理、构图和文本对齐能力，对视频数据量和算力的要求较低，也便于兼容已有的个性化模型。代价是时间建模依附于为静态图像形成的特征空间，复杂遮挡、快速形变和长程状态更容易受限。

原生时空建模会在视频 tokenizer / VAE 产生的时空表示上联合优化生成主干，使外观、运动、遮挡和镜头变化在同一训练目标中形成表示。这条路线更适合扩大时序感受野和模型规模，但需要更多、更干净的视频数据，时空 token 带来的计算压力和训练稳定性问题也更突出。视频压缩器若丢失了运动细节，后续主干没有足够信息将其还原。

两条路线并非泾渭分明。工程上常从图像权重初始化，再进行图像—视频联合预训练和视频任务微调。DiT 描述主干架构，原生视频建模描述数据、表示与训练方式：Video DiT 可以继承图像模型，3D U-Net 也可以从视频数据原生训练。短视频生成、个性化适配和有限算力更偏向图像模型视频化；追求复杂运动、长上下文和基础模型扩展性时，需要加大原生视频训练的比重。
