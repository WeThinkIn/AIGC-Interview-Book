...
能够把条件 $c$ 传入模型，只能说明接口存在。真正的任务能力要求条件能稳定改变生成分布 $p_\theta(y \mid c)$，变化发生在指定对象和时空范围内，无关内容也不应被破坏。这条链路至少包含表3-3中的四个环节。

<p align="center">表3-3 从输入条件到任务能力的必要链路</p>

| 环节 | 底层检查 | 缺失时的典型表现 |
| --- | --- | --- |
| **条件表示** | 编码器是否保留任务所需的身份、结构或时间信息 | 只生成语义相似的内容，细节、身份或节奏丢失 |
| **条件注入** | 条件在哪些网络层、去噪时间步和时空位置生效 | 条件被忽略、作用范围外泄或跨帧衰减 |
| **训练信号** | 数据中是否有对齐的条件与目标，训练目标是否为条件遵循和保持区域提供有效梯度 | 模型学会抄参考、看文本猜结果，或通过重绘整帧完成局部任务 |
| **能力验证** | 替换或移除条件后，目标属性是否发生预期变化，保持区域是否稳定 | 演示样例看起来不错，但无法确认模型是否真正使用了条件 |

以参考图为例，通用图像编码器可能保留了“人”或“狗”这类高层语义，却丢掉区分具体主体的细节。主体保持还需要细粒度参考特征、持续的跨帧注入，以及同一主体在不同姿态和视角下的训练信号。只增加一个图像输入端口，很容易得到“看起来相似”的视频，而非稳定的同一主体。
