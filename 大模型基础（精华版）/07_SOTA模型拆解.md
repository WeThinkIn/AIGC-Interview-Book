...
$$
\operatorname{Attn}(Q,K,V)=\operatorname{softmax}\left(\frac{\operatorname{Norm}(Q)\operatorname{Norm}(K)^\top}{\sqrt{d_h}}+M\right)V.
$$
...
| 分支 | 覆盖对象 | 位置规则 | 目的 |
| --- | --- | --- | --- |
| **Intra-chunk** | 同块 key | 真实相对距离 $i-j$，$ \Delta_{\text{intra}}=i-j$ | 保住**局部高分辨率** |
| **Successive-chunk** | 紧邻前一块 | 真实相对距离 $i-j$，$ \Delta_{\text{intra}}=i-j$ | 保住**跨块边界处的连续性**，避免边界断裂 |
| **Inter-chunk** | 更早的所有块 | 用**块内位置**重映射， $p_i=(i\bmod l)+l,\qquad p_j=j\bmod l,\qquad \Delta_{\text{inter}}=p_i-p_j $ | 把远距离压缩进训练分布，**防止外推失效** |
...
$$
S_t=\mathrm{Diag}(\alpha_t)\Bigl(I-\beta_t k_t k_t^\top\Bigr)S_{t-1}+\beta_t k_t v_t^\top
$$
...
- $w^{I}_{t,j}\in\mathbb{R}$ ：ead 权重（门），由 $h_t$ 投影 + 标量缩放得到 
...
$$
\mathcal{S}_t=\mathrm{Top}\text{-}k_{s\le t}\Bigl(\{I_{t,s}\}_{s=1}^{t}\Bigr),\qquad |\mathcal{S}_t|=k
$$
