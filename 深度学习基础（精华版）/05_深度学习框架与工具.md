...
**难度评分：⭐⭐⭐（3/5）| 考察频率：⭐⭐⭐⭐⭐（5/5）** 
 ...
 Dataset 负责“如何得到一个样本”，Sampler 负责“取哪些索引及其顺序”，DataLoader 负责“按批次、多进程/预取地调度”，`collate_fn` 负责“如何把样本拼成一个 batch”。
...
**难度评分：⭐⭐⭐⭐（4/5）| 考察频率：⭐⭐⭐⭐⭐（5/5）** 
 ...
 在梯度模式下，PyTorch 为 Tensor 运算记录由“反向函数（Function）—依赖 Tensor”组成的动态 DAG；`backward()` 从输出沿 DAG 应用链式法则，把梯度累加到叶子 Tensor。每次前向都会建立一张新的图，默认反向完成后释放。
...
**难度评分：⭐⭐⭐⭐（4/5）| 考察频率：⭐⭐⭐⭐⭐（5/5）** 
 ...
 Hugging Face 主要解决模型/数据/Tokenizer 和应用训练接口的复用；DeepSpeed 主要解决显存、并行和训练系统效率；Megatron-LM 主要解决 Transformer 大模型的张量/流水线等模型并行。三者可以组合，但必须明确谁负责模型实现、谁负责分布式封装、谁负责数据与训练循环。
