...
[4. AIGC 时代哪些视觉骨干最常考？](#q-014)
  ...
  - [面试问题：ViT 如何处理不同分辨率和变长视觉 token 序列？](#q-022)
  - [面试问题：FasterDiT、SiT、LightningDiT 分别从哪些方向改进 DiT？](#q-023)
  - [面试问题：DiT 如何注入时间、类别、文本和结构控制条件？](#q-024)
  - [面试问题：DiT Block 为什么常用 AdaLN、RoPE 和 In-Context Conditioning？](#q-025)
  - [面试问题：DINOv3 相比 DINOv2 的价值应如何理解？](#q-026)
  - [面试问题：ViT 中的 CLS token 有什么作用？](#q-027)
  - [面试问题：DINOv2 如何通过数据去重和检索构建高质量预训练集？](#q-028)
...
![ViT 模型架构示意图](imgs/ViT模型架构示意图.jpg)
...
<h2 id="q-022">面试问题：ViT 如何处理不同分辨率和变长视觉 token 序列？</h2>

**难度评分：⭐⭐⭐ (3/5)  |  考察频率：⭐⭐⭐⭐ (4/5)**

ViT 的标准训练通常固定图像分辨率和 patch size，因此 token 数量固定；但真实业务会遇到不同长宽比、不同分辨率和视频帧。处理这类输入，核心是让位置表示和计算图不再依赖某一个固定长度。

常见做法包括：

1. 使用二维可插值的位置编码，推理时按新的 patch 网格插值，而不是简单截断或补零。
2. 使用 RoPE、相对位置偏置或二维位置编码，让注意力根据相对空间关系工作。
3. 对高分辨率图像采用窗口注意力、分块编码或多尺度特征，控制全局注意力的二次复杂度。
4. 在多模态模型中对视觉 token 做 pooling、合并或 resampler 压缩，再交给语言模型。

这和源于 ViT 的 DiT 设计直接相关：DiT 在 VAE latent 上进行 patch 化，patch size 决定 token 数量和计算量。patch 越小，细节和空间精度通常更好，但注意力成本更高；patch 越大，推理更快，却可能损失局部结构。面试中应把“可变长度”回答成位置编码、计算复杂度和信息保真度的联合权衡，而不是只说 padding。

<h2 id="q-023">面试问题：FasterDiT、SiT、LightningDiT 分别从哪些方向改进 DiT？</h2>

**难度评分：⭐⭐⭐⭐ (4/5)  |  考察频率：⭐⭐⭐ (3/5)**

这三个方向的共同点不是简单堆叠 Transformer 层，而是分别处理 DiT 的训练信号、生成路径和视觉 tokenizer 瓶颈。

| 方法 | 主要矛盾 | 核心思路 | 工程收益 |
|---|---|---|---|
| FasterDiT | 训练收敛慢、不同噪声阶段学习难度不均 | 用 SNR 分布分析训练样本，并结合速度与方向监督 | 更快收敛，减少无效训练 |
| SiT | 固定扩散路径和采样方式限制优化空间 | 将插值函数、时间离散化、预测目标和 ODE/SDE 采样解耦 | 路径更短，降低离散化误差 |
| LightningDiT | 高维视觉 latent 重建好但扩散优化困难 | 用视觉基础模型对齐 VAE，并结合现代 Transformer、Rectified Flow 和高效训练 | 兼顾重建质量、生成质量和训练效率 |

![FasterDiT 训练与监督示意图](imgs/Faster_DiT.png)

![SiT 插值与采样示意图](imgs/SiT.png)

![LightningDiT 优化示意图](imgs/LightningDiT.png)

FasterDiT 的关键判断是：训练数据并不是在每个 timestep 上同等有价值，SNR 分布可以帮助发现模型到底在学习噪声、结构还是方向。SiT 更关注从噪声到数据的路径本身，允许把速度场、分数函数、插值和采样器分开选择。LightningDiT 则说明视觉 tokenizer 不能只看重构误差，高质量 latent 还必须适合后续生成模型优化。

面试收束可以说：**DiT 的后续创新主要沿着“监督信号更有效、生成路径更短、潜空间更适合生成、Transformer 更易部署”四条线展开。**

<h2 id="q-024">面试问题：DiT 如何注入时间、类别、文本和结构控制条件？</h2>

**难度评分：⭐⭐⭐⭐ (4/5)  |  考察频率：⭐⭐⭐⭐ (4/5)**

DiT 至少需要知道当前噪声水平，否则同一个 latent 在不同 timestep 下无法使用相同的去噪策略。最常见的做法是把 timestep 编码成向量，再通过 AdaLN 或 MLP 产生每个 Block 的缩放、偏移和门控参数。类别标签可以和时间 embedding 相加或拼接；文本条件通常经过文本编码器后，使用 cross-attention、拼接 token 或条件调制注入；深度、边缘、姿态和分割等结构条件则可通过额外 token、条件分支、残差注入或 ControlNet 风格旁路进入网络。

不同注入方式的取舍是：

- cross-attention 表达灵活，适合变长文本，但会增加注意力和显存开销；
- token 拼接结构统一，适合 In-Context Conditioning，但需要位置和模态标记；
- AdaLN 计算便宜、易于逐层调制，但对细粒度空间条件需要额外分支；
- 独立控制分支能保护预训练主干，却会增加参数和部署复杂度。

业务上要区分“语义条件”和“空间条件”：文本更擅长描述意图，边缘、深度、姿态和 mask 更擅长描述位置结构。高质量可控生成通常是二者组合，而不是单独依赖 prompt。

<h2 id="q-025">面试问题：DiT Block 为什么常用 AdaLN、RoPE 和 In-Context Conditioning？</h2>

**难度评分：⭐⭐⭐⭐ (4/5)  |  考察频率：⭐⭐⭐ (3/5)**

AdaLN 通过条件向量调节归一化后的特征，让时间步和类别等全局条件以较低成本作用于每一层；相比把条件粗暴加到输入，它可以在深层网络中持续改变特征分布。RoPE 将位置信息编码到 query/key 的相对旋转关系中，更适合表达二维 patch 的相对位置，并能在一定程度上适应长度变化。In-Context Conditioning 则把条件 token 当作上下文与图像 token 一起送入 Transformer，通过统一注意力让模型学习“条件和 latent 如何交互”，适合多条件组合和统一接口。

这些机制也有边界：AdaLN 不会自动解决精确空间对齐，RoPE 不等于真正的二维几何理解，In-Context Conditioning 会增加序列长度和注意力成本。因此应结合任务选择，不能把某一个 Block 技巧当成可控生成的全部答案。

<h2 id="q-026">面试问题：DINOv3 相比 DINOv2 的价值应如何理解？</h2>

**难度评分：⭐⭐⭐⭐ (4/5)  |  考察频率：⭐⭐⭐ (3/5)**

DINOv3 延续了 DINO 系列“无人工标签学习通用视觉表征”的主线，重点不应只记模型规模或排行榜，而要理解它如何让自监督特征在更多数据、更多分辨率和更多下游任务上保持稳定。相较 DINOv2，后续版本的价值可以从三个层面理解：更大、更干净且更有覆盖度的数据；更稳定的 teacher-student 训练与特征归一化策略；更强的密集特征质量，使分割、深度、匹配和开放词汇视觉任务受益。

这类模型的跨周期意义在于：它把视觉 backbone 从“为单一分类任务训练”推进到“作为多个视觉任务的可复用表征层”。工程落地仍要验证领域迁移、显存成本、推理延迟和下游微调数据，不能因为是 foundation model 就跳过业务评估。

<h2 id="q-027">面试问题：ViT 中的 CLS token 有什么作用？</h2>

**难度评分：⭐⭐ (2/5)  |  考察频率：⭐⭐⭐⭐ (4/5)**

CLS token 是一个可学习的全局汇聚 token，在输入序列前与所有图像 patch token 一起进入 Transformer。经过多层自注意力后，它可以从各个 patch 聚合全局信息，最后接分类头完成图像级预测。它的优势是接口稳定，无论 patch 数量多少，分类头都只读取一个向量。

CLS token 不是图像中真实存在的区域，也不等于全局平均池化。自注意力允许它主动选择哪些 patch 更重要；平均池化则对所有 patch 做固定聚合。对检测、分割和生成任务，通常还要保留空间 token 或多尺度特征，不能只保留 CLS token，否则会丢掉目标位置和局部细节。

<h2 id="q-028">面试问题：DINOv2 如何通过数据去重和检索构建高质量预训练集？</h2>

**难度评分：⭐⭐⭐⭐ (4/5)  |  考察频率：⭐⭐⭐ (3/5)**

大规模自监督预训练的瓶颈不只是图片数量，还包括重复、低质、版权不明和语义覆盖不均。DINOv2 一类视觉基础模型通常会先用图像质量规则和元数据过滤明显异常样本，再用视觉 embedding 做近重复检索与聚类，删除高度相似或同源图片，最后按语义、地域、风格和分辨率进行采样平衡。

一个可解释的数据流程是：图像预处理与质量过滤 -> 视觉特征提取 -> 近邻检索和去重 -> 聚类分析与覆盖度检查 -> 分层采样 -> 训练后用下游表现反向评估数据。去重不应只做像素 hash，因为同一图片的裁剪、缩放和压缩版本仍可能重复；语义 embedding 和局部特征结合更适合发现近重复。

这类数据工程直接决定自监督特征的泛化能力，也会影响后续检测、分割和多模态对齐。面试中可以用一句话概括：**视觉基础模型的上限不仅由 backbone 决定，也由数据的覆盖、纯度、去重和可追溯性决定。**
