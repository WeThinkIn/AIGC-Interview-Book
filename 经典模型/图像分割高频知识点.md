...
[4. SAM 为什么成为 AIGC 时代的经典视觉基础模型？](#q-014)
  ...
  - [面试问题：SAM 支持哪些 prompt，点、框和 mask 如何进入模型？](#q-021)
  - [面试问题：SAM 如何生成多个候选 mask 并判断 mask 质量？](#q-022)
  - [面试问题：SAM 系列如何轻量化并适配实时业务？](#q-023)
...
<h2 id="q-021">面试问题：SAM 支持哪些 prompt，点、框和 mask 如何进入模型？</h2>

**难度评分：⭐⭐⭐ (3/5)  |  考察频率：⭐⭐⭐⭐⭐ (5/5)**

SAM 的 prompt 大致分为稀疏提示和稠密提示。稀疏提示包括前景点、背景点和 bounding box，模型把它们编码成少量 token；稠密提示主要是已有 mask，经过卷积或下采样后变成与图像 embedding 对齐的空间特征。

点提示通常先归一化到图像坐标，再通过位置编码得到点向量，并叠加前景/背景类型 embedding；框提示可看作两个角点的组合，分别表达左上和右下边界。mask prompt 则保留更强的空间先验，适合用户在上一轮输出上继续修正。image encoder 提取的图像 embedding 可以缓存，用户每次修改 prompt 时只需重新运行轻量的 prompt encoder 和 mask decoder。

这套设计的关键不是提示种类越多越好，而是把“用户意图”转成可与图像特征交互的条件表示，因此 SAM 能支持交互式标注、图像编辑和 Agent 工具调用。

<h2 id="q-022">面试问题：SAM 如何生成多个候选 mask 并判断 mask 质量？</h2>

**难度评分：⭐⭐⭐⭐ (4/5)  |  考察频率：⭐⭐⭐⭐ (4/5)**

单个点或框可能对应多个合理解释，例如一个点落在车轮上，用户可能想分割车轮、整辆车或被遮挡的车辆。SAM 的 mask decoder 因此通常会输出多个候选 mask，而不是强行给出唯一答案；同时为每个候选输出质量预测分数，帮助系统排序或交给用户选择。

质量分数是模型对 mask 与真实目标重叠程度的估计，工程上仍需结合面积、边界平滑度、与检测框的一致性、跨帧稳定性和业务规则。对于自动标注，不能只按模型分数直接入库，而应设置阈值、抽样复核和难例回流；对于图像编辑，mask 边缘还要经过羽化、孔洞修复和前后景颜色处理，避免生成结果出现明显接缝。

<h2 id="q-023">面试问题：SAM 系列如何轻量化并适配实时业务？</h2>

**难度评分：⭐⭐⭐⭐ (4/5)  |  考察频率：⭐⭐⭐⭐ (4/5)**

SAM 的主要成本集中在 image encoder，尤其是高分辨率输入和大规模 ViT。轻量化通常沿四条路线展开：替换或蒸馏更小的 image encoder；降低输入分辨率并在局部区域高分辨率细化；缓存图像 embedding，让多次交互复用一次编码；压缩 prompt encoder 和 mask decoder，减少每次交互的延迟。

实时视频场景还要引入记忆机制和关键帧策略：关键帧运行较重的编码或重新检测，普通帧复用历史 mask 和特征，并在遮挡或置信度下降时触发纠正。部署时需要同时测量 mask 边界质量、交互延迟、显存峰值和长视频漂移，不能只看单张图像的 IoU。
