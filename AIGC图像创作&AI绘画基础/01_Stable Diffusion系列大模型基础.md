...
<h2 id="q-018b">面试问题：Rectified Flow 与传统扩散模型（score matching）的训练目标本质差异是什么？</h2>


<h2 id="q-018c">Stable Diffusion/FLUX对Rectified Flow的采样方法做了哪些优化？</h2>


<h2 id="q-018d">面试问题：介绍一下Rectified Flow的原理，与 Flow Matching、Consistency Model 是什么样的联系与区别？</h2>
...
- 基于余弦函数的平滑变化
- 在数据和噪声端都有更好的平衡
- 公式：$\bar{\alpha}_t = \cos^2\left(\frac{\pi}{2} \cdot \frac{t}{T}\right)$
...
1.  **训练分辨率与策略的优化**
Latent Diffusion仅在256×256的图像分辨率下完成全流程训练，而Stable Diffusion采用分阶段训练策略：先以256×256分辨率完成模型基础预训练，再迁移至512×512的更高分辨率下进行微调，以此适配更高清的图像生成需求。
