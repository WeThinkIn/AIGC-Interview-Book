...
[1.介绍一下AIGC图像创作大模型的完整训练与性能优化流程通常包含哪些阶段](#q-001)

[2.AIGC图像创作大模型的数据工程和标注体系如何决定模型能力上限？](#q-004)
...
<h1 id="q-001">1.介绍一下AIGC图像创作大模型的完整训练与性能优化流程通常包含哪些阶段</h1>

**难度评分：⭐⭐⭐ (3/5)  |  考察频率：⭐⭐⭐⭐⭐ (5/5)**

在AIGC时代的“中场时刻”到来后，AIGC图像创作大模型的训练流程已经不能再被简化成“收集一批图文对，然后训练一个扩散模型”。**今天的主流AIGC图像创作大模型的整体能力与高质量Caption、VLM重标注、富文本数据、编辑数据、后训练、偏好对齐、少步蒸馏和推理优化高度相关**。

Rocky认为，我们真正要抓住的是一个共同范式：**AIGC图像创作大模型的训练，不是一次性训练出一个会生成图像的大模型，而是围绕“数据、表示、生成、对齐、加速、反馈”构建一条持续迭代的训练生产线。**

从Stable Diffusion、FLUX、Qwen-Image、GLM-Image、HiDream-I1、Z-Image、Seedream以及闭源多模态图像创作系统的共同演进看，一个通用的AIGC图像创作大模型训练流程，通常可以提炼总结为九个阶段：
...
| 4. 语义标注与多模态重标注 | 用Caption模型、VLM、OCR、布局分析和人工校验生成高密度标注 | 把“图像里有什么”转化为模型可以学习的文本监督信息标签 | 详细Caption、OCR文本、空间关系、风格标签、编辑差异描述 |
...
| 6. 基础生成预训练 | 在大规模高质量图文数据上训练文生图基础大模型，学习从语义条件到图像分布的映射 | 获得通用生成能力，先让大模型“会画、能对齐、能泛化” | 基础T2I大模型、通用审美能力、基本prompt following能力 |
| 7. 持续训练与多任务能力扩展 | 加入高分辨率、文字渲染、图像编辑、多图参考、局部控制、知识图像和专业场景数据继续训练 | 从“通用会画”走向“复杂任务可用”，补齐基础预训练中缺失的能力 | 编辑能力、富文本能力、多图一致性、专业图示能力 |
| 8. SFT、偏好对齐与安全对齐 | 用精选高质量样本、人类偏好、奖励模型、VLM-as-a-Judge、DPO/GRPO/RLHF/RLAIF做后训练 | 让大模型输出更符合人类审美、指令意图、编辑保真和安全要求 | 审美提升、文字准确性提升、坏例修复、安全拒答策略 |
...
1. **先定义能力，再组织数据。**  
   今天训练AIGC图像创作大模型，不能先有数据再问大模型能做什么，而是要反过来：先定义大模型要服务什么创作场景，再决定需要什么数据。只构建普通图文对数据，大模型最多学会“看图说话式生成”；要做好文字渲染，就必须有OCR、字体、版式、多语言文本数据；要做好图像编辑，就必须有源图、目标图、编辑指令和未编辑区域约束；要做好知识图像，就必须引入图表、流程图、PPT、地图、公式和结构化标注。

2. **训练不是只训练生成主干Backbone，还要训练一整套表示系统。**  
   VAE/Tokenizer决定图像细节能否被压进Latent空间，Text Encoder/VLM决定prompt和图像条件能否被正确理解，生成主干Backbone负责在Latent空间里建模分布。很多人把训练理解为“训练DiT或U-Net扩散模型”，这是不完整的。真正的图像大模型训练，是图像表示、文本表示、多模态条件和生成网络之间的联合工程。

3. **基础预训练解决“会不会”，后训练解决“好不好用”。**  
   基础预训练让大模型具备通用生成能力，但它天然不等于产品可用。复杂prompt、准确文字、局部编辑、身份保持、审美偏好、安全边界，都需要持续训练、SFT、偏好对齐和bad case回流来解决。换句话说，预训练给大模型能力底座，后训练把能力塑造成用户真正愿意使用的形态。

4. **推理优化不是附属工作，而是产品化目标的一部分。**  
   如果一个大模型只能用几十步、很高显存、很高成本的生成一张图，它的研究价值可能很高，但产品价值会被限制。少步蒸馏、量化、缓存、编译优化、VAE tiling、Fast/Turbo版本，本质上都是把大模型能力转化成真实创作生产力。对产业大模型来说，质量、速度、成本和稳定性必须一起设计。
...
Rocky认为**AIGC图像创作大模型的竞争，不是单点架构竞争，而是谁能更系统地把数据、表示、生成、对齐、评测、部署和反馈闭环串起来。整个流程越完整，模型越不只是“能生成图像”，而是越接近一个可持续进化的视觉创作基础设施。**


<h1 id="q-004">2.AIGC图像创作大模型的数据工程和标注体系如何决定模型能力上限？</h1>
