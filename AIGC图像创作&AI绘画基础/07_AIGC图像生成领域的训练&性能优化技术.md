...
**Continued Training / CT：沿用生成损失，在新任务分布上继续扩能力。** CT通常仍然优化 $`\mathcal{L}_{\mathrm{gen}}`$，但训练数据从通用预训练集 $`\mathcal{D}_{\mathrm{pre}}`$ 切换或混合为新任务数据 $`\mathcal{D}_{\mathrm{ct}}`$，例如编辑对、多图参考、知识图像、富文本图像：

```math
\mathcal{L}_{\mathrm{CT}}=
\mathbb{E}_{(x_0,c)\sim \mathcal{D}_{\mathrm{ct}},t}
\left[
\left\|f_\theta(x_t,t,c)-y_t\right\|_2^2
\right]
```
...
```math
\mathcal{L}_{\mathrm{PE}\text{-}\mathrm{SFT}}
=
-\sum_i \log p_\theta(a_i\mid a_{\lt i},c)
```
